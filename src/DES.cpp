...
const char IP_Table[64] = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28,
...
const char IPR_Table[64] = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55,
		23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5, 45, 13, 53, 21, 61,
		29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2,
		42, 10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25 };
// expansion operation matrix
const char E_Table[48] = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11,
		12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21, 22, 23,
		24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };
// 32-bit permutation function P used on the output of the S-boxes 
const char P_Table[32] = { 16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18,
		31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19, 13, 30, 6, 22, 11, 4, 25 };
// permuted choice table (key) 
const char PC1_Table[56] = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26,
		18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36, 63, 55, 47,
		39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29,
		21, 13, 5, 28, 20, 12, 4 };
// permuted choice key (table) 
const char PC2_Table[48] = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19,
...
/*************************************************************************************** 
 *******	函数功能：DES/3DES加解密处理函数         								**** 
 *******	函数参数：																**** 
 *******			 bool bType:加解密标志；ENCRYPT(0)加密;DECRYPT(1)解密			**** 
 *******			 char* In:要进行DES/3DES加解密的原始数据必须为8的倍数			**** 
 *******			 unsigned datalen:数据长度  									**** 
 *******			 const char* Key：用来加解密的密钥[8字节为DES、16字节为3DES)    ****
 *******			 const unsigned char keylen：密钥长度，8或16 .					****
 *******			 char *Out：(输出)加密为生成的密文；解密为生成的明文            ****
 *******	返 回 值：成功返回：true(非零);失败返回：false(零)						****
 ***************************************************************************************/
bool CDES::RunDes(bool bType, char* In, unsigned datalen, const char* Key,
		const unsigned char keylen, char* Out) {
	bool bMode = 0;  // 将bMode 参数直接设置成：0，即ECB模式
...
/*
 ...
 返回值 说明：	bool	:是否填充成功
 *******************************************************************/
bool CDES::RunPad(int nType, const char* In, unsigned datalen, char* Out,
		unsigned& padlen) {
